...
from ..core.model_compatibility import check_all_models, get_recommended_models
    ...
    def run(self):
        try:
            results = fit_all_models(
                self.phi, self.k, self.selected, self.custom, mode=self.mode
            )
            for name, res in results.items():
                if res["success"] and res.get("y_pred") is not None:
                    ...
                    res.update({"r2": np.nan, "rmse": np.nan,
...
class MultiFileWorker(QThread):
    ...
    def __init__(self, files_data, selected_models, custom_params=None,
                 mode="nanothermite"):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            for idx, (filename, phi, k, df) in enumerate(self.files_data):
                ...
                file_results = fit_all_models(
                    phi, k, self.selected_models, self.custom_params,
                    mode=self.mode
                )
...
class MainWindow(QMainWindow):
    def __init__(self, mode="nanothermite"):
        ...
        # ===== WINDOW SETUP =====
        self.setWindowTitle(f"{self.mode_icon} Thermal Conductivity Modeling Suite v2.0")
    ...
    def _get_global_style(self):
        return """
            ...
            QMenu {
                background-color: #2c3e50; color: #ecf0f1;
                border: 1px solid #34495e; border-radius: 6px; padding: 5px;
            }
            QMenu::item { padding: 8px 30px 8px 20px; border-radius: 4px; }
            QMenu::item:selected { background-color: #34495e; }
            QMenu::separator {
                height: 1px; background-color: #34495e; margin: 4px 10px;
            }

            QToolBar {
                background: qlineargradient(x1: 0, y1: 0, x2: 1, y2: 0,
                                          stop: 0 #2c3e50, stop: 1 #34495e);
                border: none; padding: 6px 10px; spacing: 6px;
            }
            QToolBar QLabel { color: #ecf0f1; font-weight: bold; }
            QToolBar QPushButton {
                background-color: rgba(255, 255, 255, 0.1);
                color: #ecf0f1; border: none; border-radius: 5px;
            ...
            QGroupBox {
                font-weight: bold; border: 2px solid #d5d8dc;
            ...
            QTableWidget {
                gridline-color: #d5d8dc; background-color: white;
            ...
            QHeaderView::section {
                ...
                padding: 8px; font-weight: bold; font-size: 11px; border: none;
            }

            QCheckBox { spacing: 8px; color: #2c3e50; font-size: 11px; }
            QCheckBox::indicator {
                width: 22px; height: 22px; border-radius: 4px;
                border: 2px solid #bdc3c7; background: white;
            }
            QCheckBox::indicator:hover { border: 2px solid #3498db; }
            QCheckBox::indicator:checked {
                border: 2px solid #27ae60; background: #27ae60;
            }
            QCheckBox::indicator:checked:hover {
                border: 2px solid #2ecc71; background: #2ecc71;
            }

            QComboBox {
                padding: 6px 12px; border: 2px solid #d5d8dc;
                border-radius: 6px; background: white; color: #2c3e50;
            ...
            QLineEdit {
                padding: 6px 10px; border: 2px solid #d5d8dc;
            ...
            QDoubleSpinBox, QSpinBox {
                padding: 6px 10px; border: 2px solid #d5d8dc;
                border-radius: 6px; background: white;
                color: #2c3e50; font-size: 11px; min-height: 20px;
            }
            QDoubleSpinBox:focus, QSpinBox:focus { border-color: #3498db; }

            QStatusBar {
                background: #2c3e50; color: #ecf0f1; border-top: none;
            }
            QStatusBar QLabel { color: #ecf0f1; font-size: 11px; }

            QProgressBar {
                border: none; border-radius: 4px;
                background: #34495e; text-align: center; color: white;
                font-weight: bold; font-size: 10px; height: 18px;
            }
            QProgressBar::chunk {
                background: qlineargradient(x1: 0, y1: 0, x2: 1, y2: 0,
            ...
            QPushButton {
                ...
                font-weight: bold; padding: 8px 16px; font-size: 11px;
            }
            ...
            QScrollBar::handle:vertical {
                background: #bdc3c7; border-radius: 6px; min-height: 30px;
            }
            QScrollBar::handle:vertical:hover { background: #95a5a6; }
            QScrollBar:horizontal {
                background: #ecf0f1; height: 12px; border-radius: 6px;
            }
            QScrollBar::handle:horizontal {
                background: #bdc3c7; border-radius: 6px; min-width: 30px;
            }
            QScrollBar::handle:horizontal:hover { background: #95a5a6; }

            QSplitter::handle { background: #d5d8dc; width: 4px; }
    ...
    def _create_toolbar(self):
        ...
        for mode_id in available_modes:
            ...
            self.mode_combo.addItem(
                f"{mode_info.get('icon', '🔬')} {mode_info.get('name', mode_id)}"
            )

        self.mode_combo.setCurrentIndex(
            available_modes.index(self.current_mode)
            if self.current_mode in available_modes else 0
        )
    ...
    def _build_data_tab(self):
        ...
        self.data_info_label.setStyleSheet("""
            ...
            background: #f0f4f8; border: 1px solid #d5d8dc; font-size: 12px;
        """)
    ...
    def _build_fit_tab(self):
        ...
        vertical_splitter.setStyleSheet("""
            QSplitter::handle {
                background: #d5d8dc; height: 5px; margin: 2px;
            }
            QSplitter::handle:hover { background: #27ae60; }
        """)
        ...
        group_models.setStyleSheet("""
            QGroupBox {
                font-weight: bold; border: 2px solid #d5d8dc;
                border-radius: 8px; padding-top: 14px; background: white;
            }
            QGroupBox::title { color: #2c3e50; padding: 0 8px; }
        """)
        ...
        self.category_combo.addItems(
            ["All Models"] + list(self.MODEL_CATEGORIES.keys())
        )
        ...
        select_all.setStyleSheet("""
            QPushButton {
                ...
                border: none; border-radius: 4px;
                padding: 3px 10px; font-weight: bold; font-size: 10px;
            }
        ...
        reset_btn.setStyleSheet("""
            QPushButton {
                ...
                border: none; border-radius: 4px;
                padding: 2px 10px; font-weight: bold; font-size: 10px;
            }
        ...
        self.legend_format_combo.currentTextChanged.connect(
            self.change_legend_format
        )
        ...
        graph_splitter.setStyleSheet("""
            QSplitter::handle {
                background: #d5d8dc; width: 5px; margin: 2px;
            }
            QSplitter::handle:hover { background: #27ae60; }
        """)
        ...
        try:
            ...
            self.canvas_resid.setSizePolicy(
                QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
            )
            self.toolbar_resid = NavigationToolbar2QT(self.canvas_resid, self)
            self.toolbar_resid.setStyleSheet(
                "background: #ecf0f1; border: none;"
            )
    ...
    def _build_models_tab(self):
        ...
        self.models_text.setStyleSheet("""
            QTextEdit {
                ...
                background-color: white; border: 1px solid #d5d8dc;
    ...
    def _build_models_html(self):
        ...
        <div style="background: linear-gradient(135deg, #2c3e50 0%, #34495e 100%);
            ...
            <p style="color: #bdc3c7; margin: 5px 0 0 0;">
                ...
                &nbsp; • &nbsp; k<sub>matrix</sub> = {self.k_matrix:.2f} W/m·K ({self.matrix_name})
                &nbsp; • &nbsp; k<sub>filler</sub> = {self.k_filler:.2f} W/m·K ({self.filler_name})
            </p>
        </div>
        """
        ...
        # ====================================================================
        # MODE-SPECIFIC RECOMMENDATION BANNER
        # ====================================================================
        if self.current_mode == "nanothermite":
            html += f"""
            <div style="background: #fdedec; border-left: 4px solid #c0392b; padding: 12px 15px; border-radius: 6px; margin-bottom: 20px;">
                <b style="color: #c0392b;">🔥 Nanothermite Recommendations</b>
                <p style="margin: 6px 0 0 0; font-size: 11px; color: #2c3e50;">
                    <b>Recommended defaults:</b>
                    <span style="color: #27ae60; font-weight: bold;">Maxwell-Eucken</span>,
                    <span style="color: #27ae60; font-weight: bold;">Cheng-Vachon</span>,
                    <span style="color: #27ae60; font-weight: bold;">Baschirow-Selenew</span><br>
                    <b>Avoid:</b> Parallel, Lewis-Nielsen, Maxwell-Eucken Upper (they assume k<sub>filler</sub> &gt; k<sub>matrix</sub>)
                </p>
            </div>
            """
        else:
            html += f"""
            <div style="background: #eafaf1; border-left: 4px solid #27ae60; padding: 12px 15px; border-radius: 6px; margin-bottom: 20px;">
                <b style="color: #1e8449;">🧪 Polymer Composite Recommendations</b>
                <p style="margin: 6px 0 0 0; font-size: 11px; color: #2c3e50;">
                    <b>Recommended defaults:</b>
                    <span style="color: #27ae60; font-weight: bold;">Agari</span>,
                    <span style="color: #27ae60; font-weight: bold;">Lewis-Nielsen</span>,
                    <span style="color: #27ae60; font-weight: bold;">Maxwell-Eucken</span><br>
                    <b>Avoid:</b> Series, Maxwell-Eucken Lower (they assume k<sub>filler</sub> &lt; k<sub>matrix</sub>)
                </p>
            </div>
            """
        ...
        <div style="margin-top: 20px; padding: 12px; background: #f8f9fa; border-radius: 6px; text-align: center; font-size: 11px; color: #95a5a6;">
            Thermal Conductivity Modeling Suite v2.0 • {self.mode_label} Mode • Generated: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M')}
        </div>
        """
    ...
    def _build_stats_tab(self):
        ...
        params_label.setStyleSheet(
            "font-weight: bold; font-size: 14px; color: #2c3e50; margin-top: 10px;"
        )
    ...
    def _build_report_tab(self):
        ...
        self.report_preview.setStyleSheet("""
            QTextEdit {
                background-color: white; border: 1px solid #d5d8dc;
                border-radius: 6px; padding: 15px; font-size: 12px;
            }
        """)
    ...
    def _update_mode_ui(self):
        if self.current_mode == "nanothermite":
            header_labels = ["#", "φ (Filler)", "k_meas (W/m·K)",
                             "k_std_dev", "1-φ (Matrix)"]
            info_text = "🔥 <b>Nanothermite Mode</b> — φ = Oxidizer (filler), (1-φ) = Reductant (matrix)"
        else:
            header_labels = ["#", "φ (Filler)", "k_meas (W/m·K)",
                             "k_std_dev", "1-φ (Matrix)"]
            info_text = "🧪 <b>Polymer Mode</b> — φ = Filler, (1-φ) = Polymer Matrix"

        self.data_table.setHorizontalHeaderLabels(header_labels)
        self.data_info_label.setText(info_text)

    # ========================================================================
    # MODEL POPULATION — MODE-SPECIFIC DEFAULTS
    # ========================================================================

    def _populate_models_by_category(self):
        """Populate the model selection grid with mode-specific defaults."""
        # Only reload if registry is empty or doesn't have discovered models
        if not self.MODEL_REGISTRY or len(self.MODEL_REGISTRY) <= 26:
        ...
        # ====================================================================
        # POLYMER (k increases with φ): Agari, Lewis-Nielsen, Maxwell-Eucken
        # NANOTHERMITE (k decreases with φ): Maxwell-Eucken, Cheng-Vachon,
        #                                     Baschirow-Selenew
        # ====================================================================
        if self.current_mode == "polymer":
            default_models = ["Agari", "Lewis-Nielsen", "Maxwell-Eucken"]
        else:
            default_models = ["Maxwell-Eucken", "Cheng-Vachon",
                              "Baschirow-Selenew"]
        ...
        # Debug
        print(f"[DEBUG] Populated {len(self.model_checks)} models in UI ({self.current_mode} mode)")
        print(f"[DEBUG] Defaults checked: {[n for n, cb in self.model_checks.items() if cb.isChecked()]}")
    ...
    def _on_tab_changed(self, index):
        ...
        if tab_text in ("Models Reference", "📚 Models Reference"):
            self._update_models_reference()
    ...
    def update_plots(self):
        if self.df is None:
            self.canvas_main.axes.clear()
            self.canvas_main.axes.text(
                0.5, 0.5, "Load data first",
                ha='center', va='center',
                transform=self.canvas_main.axes.transAxes, fontsize=14
            )
            self.canvas_main.draw()
            self.canvas_resid.axes.clear()
            self.canvas_resid.draw()
        ...
        if not results:
            self.canvas_main.axes.clear()
            self.canvas_main.axes.text(
                0.5, 0.5, "No fit results. Run analysis.",
                ha='center', va='center',
                transform=self.canvas_main.axes.transAxes, fontsize=14
            )
            self.canvas_main.draw()
    ...
    def update_param_table(self):
        try:
            ...
            has_params = any(res.get("params") and res.get("success", False)
            ...
            for name, res in results.items():
                if res.get("params") and res.get("success", False):
                    ...
                    for (pname, pval), std_err in zip(res["params"].items(), perr):
                        ...
                        if std_err is not None and np.isfinite(std_err):
        ...
        try:
            ...
            if not results:
                self.report_preview.setHtml(
                    ...
                    "No results available. Run analysis first.</p>"
                )
                return
            ...
            if not filtered_results:
                self.report_preview.setHtml(
                    "<p style='color:#7f8c8d; text-align:center;'>"
                    "No valid model fits available.</p>"
                )
                return
            ...
            for name, res in sorted_results:
                if res.get("success", False):
                    ...
                    if r2 > best_r2:
                        best_r2 = r2
                        best_name = name
                        best_rmse = res.get('rmse', 0)
                        best_mae = res.get('mae', 0)

            if self.current_mode == "nanothermite":
                convention = (f"φ = {self.filler_name} (filler), "
                              f"(1-φ) = {self.matrix_name} (matrix)")
            else:
                convention = (f"φ = {self.filler_name} (filler), "
                              f"(1-φ) = {self.matrix_name} (polymer matrix)")
            ...
            if best_name and best_r2 > 0:
                ...
                R² = {best_r2:.4f} | RMSE = {best_rmse:.4f} W/m·K | MAE = {best_mae:.4f} W/m·K
                </p>"""

            html += f"<hr><p style='color:#7f8c8d;font-size:11px;'>Generated by Thermal Conductivity Modeling Suite v2.0 - {self.mode_label} Mode | Oukil Khaled</p>"
    ...
    def _set_buttons_enabled(self, enabled):
        for child in self.findChildren(QPushButton):
            if child.text() in ["▶ Run", "⚙️ Params", "📂 Load",
                                "📂 Load Multi", "🧠 Smart", "🔍 Discover"]:
        ...
        try:
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                if result:
                    ...
                    self.k_matrix = result.get(
                        'k_matrix',
                        get_k_value(self.current_mode, "matrix", self.matrix_name)
                    )
                    self.k_filler = result.get(
                        'k_filler',
                        get_k_value(self.current_mode, "filler", self.filler_name)
                    )
                    ...
                    self.status.showMessage(
                        f"Loaded {len(self.df)} data points from "
    ...
    def load_multiple_files(self):
        ...
        self._populate_models_by_category()
        self.status.showMessage(
            f"Loaded {len(self.df)} data points from {len(paths)} files"
        )
        ...
        QMessageBox.information(
            self, "Success",
            f"✅ Loaded {len(paths)} files successfully!\n"
    ...
    def add_manual_row(self):
        try:
            if self.multi_files and len(self.multi_files) > 0:
                ...
                self.df = pd.concat(
                    [df_updated for _, df in self.multi_files], ignore_index=True
                )
            else:
                if self.df is not None:
                    new_row = pd.DataFrame({
    ...
    # ========================================================================
    # ANALYSIS — WITH COMPATIBILITY CHECK
    # ========================================================================
    ...
    def run_analysis(self):
        if self.df is None or len(self.df) < 2:
            QMessageBox.warning(
                self, "Warning",
                "Please load experimental data first (minimum 2 points)."
            )
            return
        ...
        data_trend = compat["data_trend"]

        # ---------- Hard error: incompatible models ----------
        if incompatible:
            recommended = get_recommended_models(self.current_mode)

            dlg = CompatibilityDialog(
                title="Incompatible Models Detected",
                subtitle="",
                data_trend=data_trend,
                incompatible_models=incompatible,
                warned_models=warnings,
                compatibility_results=compat["results"],
            ...
            if not dlg.user_wants_to_continue():
                self.status.showMessage(
                    f"Analysis cancelled — {len(incompatible)} "
                    f"incompatible model(s) detected"
                )
                return
        ...
        self.worker = FitWorker(phi, k, selected, self.custom_params,
                                mode=self.current_mode)
    ...
    def _on_analysis_finished(self, results):
        ...
        self.status.showMessage(
            f"Analysis complete: {successes}/{len(results)} "
            f"models fitted successfully"
        )

        self.legend_format = (
            "compact" if self.legend_format_combo.currentText() == "Compact"
            else "full"
        )
        ...
        if hasattr(self, 'tab_best_r2'):
            if self.multi_file_results and len(self.multi_file_results) > 0:
                self.tab_best_r2.update_data(
                    self.multi_file_results, self.fit_results, self.df
                )
                self._enable_best_r2_tab(True)
                ...
                self._enable_best_r2_tab(True)
                self.tab_best_r2.interpretation_text.setText(
                    "📋 Multiple files are loaded but no multi-file "
                    "analysis has been run.\n\n"
                    "To populate this tab:\n"
                    "1. Make sure models are selected\n"
                    "2. Click 'Run Analysis' and select 'Yes' for "
                    "multi-file analysis\n"
                    "3. Wait for the analysis to complete"
                )
            else:
        ...
        best = max(
            results.items(),
            key=lambda x: x[1].get("r2", -1) if x[1]["success"] else -1
        )
        if best[1]["success"]:
            QMessageBox.information(
    ...
    def run_multi_file_analysis(self):
        ...
        self.multi_worker = MultiFileWorker(
            files_data, selected, self.custom_params, mode=self.current_mode
        )
    ...
    def _on_multi_file_finished(self, results):
        ...
        if selected and self.df is not None:
            phi = self.df["Filler_volume_fraction"].values
            k = self.df["k_meas"].values
            combined_results = fit_all_models(
                phi, k, selected, self.custom_params, mode=self.current_mode
            )
        ...
        self.update_report_preview()
        self.status.showMessage(
            f"Multi-file analysis complete: {len(results)} files analyzed"
        )
    ...
    def _generate_multi_file_summary(self, results):
        ...
        for filename, file_results in results.items():
            for model_name, result in file_results.items():
                if result.get("success", False):
                    r2 = result.get("r2", -1)
                    model_scores.setdefault(model_name, []).append(r2)
        ...
        if model_scores:
            ...
            for model_name, scores in model_scores.items():
                avg_r2 = float(np.mean(scores)) if scores else 0
                std_r2 = float(np.std(scores)) if scores else 0
            ...
            for i, (model_name, avg_r2, std_r2, n) in enumerate(avg_scores[:10], 1):
                marker = "🏆" if i == 1 else "  "
                summary_lines.append(
                    f"{marker} {i:2d}. {model_name:<35} "
                    f"R² = {avg_r2:.4f} ± {std_r2:.4f} (n={n})"
                )
    ...
    def open_parameter_dialog(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if custom is not None:
                ...
                self.status.showMessage(
                    f"Parameters configured for {len(custom)} model(s)"
                )
                QMessageBox.information(
                    self, "Parameters Saved",
                    f"Custom parameters configured for {len(custom)} model(s).\n"
                    f"Click 'Run Analysis' to execute fitting."
                )
    ...
    def open_model_discovery(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error",
                f"Failed to open model discovery:\n{str(e)}\n"
    ...
    def _refresh_after_discovery(self):
        ...
        discovered_models = [name for name, meta in self.MODEL_REGISTRY.items()
                             if meta.get("discovered", False)]
        self.MODEL_CATEGORIES["Discovered Models"] = discovered_models
        print(f"[DEBUG] Discovered models: {discovered_models}")
        ...
        if count > 0:
            self.status.showMessage(
                f"✅ {count} new model(s) discovered and added to the registry!"
            )
    ...
    def open_generate_data(self):
        try:
            ...
            QMessageBox.critical(
                self, "Error",
                f"Failed to open Generate Data dialog:\n{str(e)}"
            )
        except Exception as e:
            QMessageBox.critical(self, "Error",
                                 f"An error occurred:\n{str(e)}")
    ...
    def save_graph(self):
        if self.df is None:
            QMessageBox.warning(
                self, "Warning",
                "No data loaded. Please load data and run analysis first."
            )
            return
    ...
    def export_results(self):
        if not self.fit_results:
            QMessageBox.warning(
                self, "Warning",
                "No results to export. Run analysis first."
            )
            return
        path, _ = QFileDialog.getSaveFileName(
        ...
        try:
            ...
            self.status.showMessage(
                f"Results exported to {os.path.basename(path)}"
            )
            QMessageBox.information(self, "Success",
    ...
    def _do_export(self, fmt):
        if fmt == "excel":
            path, _ = QFileDialog.getSaveFileName(
                self, "Export to Excel", "results.xlsx", "Excel (*.xlsx)"
            )
            if path:
                export_excel(path, self.df, self.fit_results)
                self.status.showMessage(
            ...
            path, _ = QFileDialog.getSaveFileName(
                self, "Export to CSV", "results.csv", "CSV (*.csv)"
            )
            if path:
                export_csv(path, self.df, self.fit_results)
                self.status.showMessage(
                    f"CSV exported: {os.path.basename(path)}"
                )
    ...
    def generate_report(self):
        if not self.fit_results:
            QMessageBox.warning(
                self, "Warning",
                "No results to report. Run analysis first."
            )
            return
        path, _ = QFileDialog.getSaveFileName(
            self, "Save PDF Report", "report.pdf", "PDF (*.pdf)"
        )
        if not path:
            return
        try:
            ...
            meta = {
                ...
                "Software": (
                    f"Thermal Conductivity Modeling Suite v2.0 - "
            ...
            generate_pdf_report(
                path, self.df, self.fit_results, meta,
                [fig_main, fig_resid], mode=self.current_mode
            )
            self.status.showMessage(
                f"PDF report saved to {os.path.basename(path)}"
            )
            QMessageBox.information(
                self, "Success",
                f"Professional PDF report generated.\n{path}"
            )
        except Exception as e:
            QMessageBox.critical(self, "Error",
