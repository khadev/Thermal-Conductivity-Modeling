...
# 🔬 Thermal Conductivity Modeling Suite v3.0
...
### v3.0.0 (Current) — Physics Correctness Release
- ✅ **Fixed φ-convention bug** in `models.py` — removed `phi_filler = 1.0 - phi` 
     from ~17 Group B models (Percolation, Lewis-Nielsen, EMT, Geometric Mean, 
     Cheng-Vachon, Baschirow-Selenew, Tsao, Hamilton-Crosser, GEM, Rayleigh, 
     Halpin-Tsai, Hatta, Modified Hashin-Shtrikman, Hamilton-Crosser Extended, 
     Maxwell-Eucken Upper, Russell, Ce Wen Nan)
- ✅ **Fixed `de_loor_model`** — rewrote the correct De Loor equation 
     (previous version returned 0.0)
- ✅ **Fixed `bruggeman_model`** — replaced `fsolve` with `brentq` for 
     bounded root-finding (previous version returned NaN for polymer composites)
- ✅ **Fixed `modified_hashin_shtrikman_model`** — added proper k normalization 
     (previous version was unresponsive to k)
- ✅ **Fixed `MODEL_DIRECTION`** in `model_compatibility.py` — made 
     Parallel, Series, Maxwell-Eucken `"flexible"` (they flip direction 
     depending on k_filler vs k_matrix)
- ✅ **Fixed compatibility sets** — no longer rejects correct models in 
     their own mode
- ✅ **Fixed `optimizer.py`** — now passes `k_matrix`/`k_filler` through 
